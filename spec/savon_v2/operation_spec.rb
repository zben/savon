...
describe SavonV2::Operation do
  ...
  def new_operation(operation_name, wsdl, globals)
    SavonV2::Operation.create(operation_name, wsdl, globals)
  end
  ...
  describe ".create with a WSDL" do
    ...
    it "raises if the operation is not available for the service" do
      expect { new_operation(:no_such_operation, wsdl, globals) }.
        to raise_error(SavonV2::UnknownOperationError, /Unable to find SOAP operation: :no_such_operation/)
    end
  end
  ...
  describe "#build" do
    it "returns the Builder" do
      ...
      expect(builder).to be_a(SavonV2::Builder)
  ...
  describe "#call" do
    it "returns a response object" do
      operation = new_operation(:verify_address, wsdl, globals)
      expect(operation.call).to be_a(SavonV2::Response)
    end
    ...
    it "returns a SavonV2::Multipart::Response if available and requested globally" do
      ...
      with_multipart_mocked do
        ...
        expect(response).to be_a(SavonV2::Multipart::Response)
      end
    end

    it "returns a SavonV2::Multipart::Response if available and requested locally" do
      with_multipart_mocked do
        operation = new_operation(:authenticate, no_wsdl, globals)
    ...
    it "raises if savon_v2-multipart is not available and it was requested globally" do
      globals.multipart true

      operation = new_operation(:authenticate, no_wsdl, globals)

      expect { operation.call }.
        to raise_error RuntimeError, /Unable to find SavonV2::Multipart/
    end

    it "raises if savon_v2-multipart is not available and it was requested locally" do
      operation = new_operation(:authenticate, no_wsdl, globals)
  ...
  def with_multipart_mocked
    ...
    SavonV2.const_set('Multipart', multipart_mock)

    yield
  ensure
    SavonV2.send(:remove_const, :Multipart) if SavonV2.const_defined? :Multipart
  end
