...
describe SavonV2::Response do

  let(:globals) { SavonV2::GlobalOptions.new }
  let(:locals)  { SavonV2::LocalOptions.new }

  describe ".new" do
    it "should raise a SavonV2::Fault in case of a SOAP fault" do
      expect { soap_fault_response }.to raise_error(SavonV2::SOAPFault)
    end

    it "should not raise a SavonV2::Fault in case the default is turned off" do
      globals[:raise_errors] = false
      expect { soap_fault_response }.not_to raise_error
    end

    it "should raise a SavonV2::HTTP::Error in case of an HTTP error" do
      expect { soap_response :code => 500 }.to raise_error(SavonV2::HTTPError)
    end

    it "should not raise a SavonV2::HTTP::Error in case the default is turned off" do
      globals[:raise_errors] = false
  ...
  describe "#soap_fault" do
    ...
    it "should return a SOAPFault in case of a SOAP fault" do
      expect(soap_fault_response.soap_fault).to be_a(SavonV2::SOAPFault)
    end
  end
  ...
  describe "#http_error" do
    ...
    it "should return a HTTPError in case of an HTTP error" do
      expect(soap_response(:code => 500).http_error).to be_a(SavonV2::HTTPError)
    end
  end
  ...
  describe "#header" do
    ...
    it "should throw an exception when the response header isn't parsable" do
      expect { invalid_soap_response.header }.to raise_error SavonV2::InvalidResponseError
    end
  end
