...
describe "Options" do
  ...
  context "global :env_namespace" do
    ...
    it "when not set, SavonV2 defaults to use :env as the namespace identifier for the SOAP envelope" do
  ...
  context "global: raise_errors" do
    it "when true, instructs SavonV2 to raise SOAP fault errors" do
      ...
      expect { client.call(:authenticate, :xml => Fixture.response(:soap_fault)) }.
        to raise_error(SavonV2::SOAPFault)
    ...
    it "when true, instructs SavonV2 to raise HTTP errors" do
      client = new_client(:endpoint => @server.url(404), :raise_errors => true)
      expect { client.call(:authenticate) }.to raise_error(SavonV2::HTTPError)
    end

    it "when false, instructs SavonV2 to not raise SOAP fault errors" do
    ...
    it "when false, instructs SavonV2 to not raise HTTP errors" do
  ...
  context "global :log" do
    it "instructs SavonV2 not to log SOAP requests and responses" do
      stdout = mock_stdout {
    ...
    it "instructs SavonV2 to log SOAP requests and responses" do
  ...
  context "request: message_tag" do
    ...
    it "without it, SavonV2 tries to get the message tag from the WSDL document" do
