...
describe SavonV2::Model do
  ...
  describe ".client" do
    it "returns the memoized client" do
      model = Class.new {
        extend SavonV2::Model
        client :wsdl => Fixture.wsdl(:authentication)
      }

      expect(model.client).to be_a(SavonV2::Client)
      expect(model.client).to equal(model.client)
    end

    it "raises if the client was not initialized properly" do
      model = Class.new { extend SavonV2::Model }

      expect { model.client }.
        to raise_error(SavonV2::InitializationError, /^Expected the model to be initialized/)
    end
  end
